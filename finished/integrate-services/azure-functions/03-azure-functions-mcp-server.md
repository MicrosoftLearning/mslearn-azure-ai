...
1. Press **Ctrl+Shift+P** to open the Command Palette. Run the **Azure Functions: Create Function...** command and choose the following options when prompted:
    ...
    - **Template:** Skip for now (you define functions in the next section)
...
1. Open *host.json* in the Explorer sidebar and replace its contents with the following configuration. The **extensionBundle** section was already generated by the extension. The **mcpToolTrigger** section defines the MCP server name, version, and instructions that MCP clients display when connecting.
...
In this section you create the Python virtual environment and install the project dependencies.

1. In the menu bar select **Terminal > New Terminal** to open a terminal window in VS Code.

1. Run the following command to create the Python virtual environment.

    ```bash
    python -m venv .venv
    ```

1. Run the following command to activate the Python virtual environment. **Note:** On Linux/macOS, use the Bash command. On Windows, use the PowerShell command. If using Git Bash on Windows, use **source .venv/Scripts/activate**.
...
1. The MCP SSE endpoint is available at `http://localhost:7071/runtime/webhooks/mcp/sse`. Create a *.vscode/mcp.json* file in your project to register this endpoint as an MCP server in Visual Studio Code. This file tells Visual Studio Code where to find MCP servers and how to connect to them. Create the file with the following content:
...
1. After saving *.vscode/mcp.json*, Visual Studio Code detects the MCP server configuration. Open GitHub Copilot chat and switch to **Agent** mode by selecting the mode dropdown at the top of the chat panel. Select the tools icon (wrench) and verify that both **summarize_text** and **classify_document** appear in the available tools list with their descriptions.

1. Test the **classify_document** tool by entering the following prompt in the Copilot chat:
    ...
    Copilot invokes the tool and returns a response. Verify the result contains a classification of **contract** — the function selects the first category in the comma-separated list that best applies as a match. Check the terminal output for the function invocation log entry.
...
1. Now test the tools with natural language prompts that don't name the tool directly. In a real scenario, MCP clients discover available tools and decide which one to invoke based on the tool descriptions. Enter the following prompt in the Copilot chat:

    ```
    Is the following text a contract, invoice, or memo? 'Invoice #1234 for services rendered in January 2026. Total amount due: $5,000.'
    ```
...
1. Enter the following prompt to test natural tool discovery for the **summarize_text** tool:
    ...
    >**Note:** With natural prompts, Copilot may occasionally answer directly without invoking the MCP tool. If a tool is not invoked, try rephrasing the prompt or explicitly name the tool. This behavior is expected — the language model decides whether a tool is needed based on the prompt and the tool descriptions.
...
- Confirm the virtual environment is activated before running the app or starting the debugger.
